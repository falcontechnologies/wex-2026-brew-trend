...
BASE_URL  = "https://formulae.brew.sh/api/analytics/install/30d.json"
DEFAULT_DB = "brew_data.db"
...
def init_db(conn: sqlite3.Connection) -> None:
    # For brew_data.db, tables already exist with this schema:
    # snapshots: id, run_date, run_time, category, total_count, start_date, end_date
    # installations: id, snapshot_id, rank, formula, count, percent
    pass  # DB already initialized by fetch_homebrew.py
...
def fetch_and_store(force: bool = False) -> dict:
    ...
    with get_conn() as conn:
        ...
        existing = conn.execute(
            "SELECT id FROM snapshots WHERE run_date=? AND start_date=? AND end_date=?",
            (fetch_date, payload.get("start_date"), payload.get("end_date")),
        ).fetchone()
        if existing:
            if not force:
                return {"ok": False, "message": f"Snapshot for {fetch_date} already exists (id={existing['id']}). Use force=true to replace."}
            conn.execute("DELETE FROM installations WHERE snapshot_id=?", (existing["id"],))
            conn.execute("DELETE FROM snapshots WHERE id=?",          (existing["id"],))
            conn.commit()

        cur = conn.execute(
            "INSERT INTO snapshots (run_date,run_time,category,total_count,start_date,end_date) VALUES (?,?,?,?,?,?)",
            (fetch_date, fetched_at[11:19], payload.get("category"), payload.get("total_count"), 
        ...
        conn.executemany(
            "INSERT INTO installations (snapshot_id,rank,formula,count,percent) VALUES (?,?,?,?,?)", rows)
        conn.commit()
...
def api_installs(snap_id):
    ...
    with get_conn() as conn:
        ...
        total     = conn.execute(f"SELECT COUNT(*) FROM installations WHERE {where}", params).fetchone()[0]
        max_count = conn.execute(f"SELECT MAX(count) FROM installations WHERE snapshot_id=?", [snap_id]).fetchone()[0] or 1
        rows      = conn.execute(
            f"SELECT rank,formula,count,percent FROM installations WHERE {where} ORDER BY rank LIMIT ? OFFSET ?",
...
def api_max_count(snap_id):
    with get_conn() as conn:
        row = conn.execute("SELECT MAX(count) as mc FROM installations WHERE snapshot_id=?", [snap_id]).fetchone()
...
def api_timeseries():
    ...
    with get_conn() as conn:
        snap_q  = "SELECT id,run_date,start_date,end_date FROM snapshots"
        snap_p  = []
        snap_q += " ORDER BY run_date ASC"
        ...
        for f in formulae:
            ...
            for s in snaps:
                row = conn.execute(
                    "SELECT count,rank,percent FROM installations WHERE snapshot_id=? AND formula=?",
                    [s["id"], f]).fetchone()
                pts.append({"snap_id": s["id"], "run_date": s["run_date"],
...
def api_export(snap_id):
    ...
    with get_conn() as conn:
        rows = conn.execute(
            f"SELECT rank,formula,count,percent FROM installations WHERE {where} ORDER BY rank",
            params).fetchall()
...
def api_fetch():
    force = bool(request.get_json(force=True).get("force", False))
    with _fetch_lock:
        result = fetch_and_store(force)
...
function renderOverview() {
  ...
  document.getElementById('ov-stats').innerHTML = `
    <div class="stat-card"><div class="label">Total installs</div><div class="value">${fmtNum(s.total_count)}</div><div class="sub">30-day window</div></div>
    ...
    <div class="stat-card"><div class="label">Snapshot ID</div><div class="value">#${s.id}</div><div class="sub">${s.category||'—'}</div></div>`;
}
...
function renderTsRawTable() {
  ...
  const hdr = `<th>Formula</th>` + snaps.map(s=>`<th>${s.fetch_date}<br><span style="color:var(--muted);font-size:.6rem">30d</span></th>`).join('');
...
function exportTsData(fmt) {
  ...
  if (fmt==='json') {
    ...
    const keys = ['formula','fetch_date','start_date','end_date','count','rank','percent'];
...
function populateCompareSelects() {
  ['cmp-a','cmp-b'].forEach((id,i)=>{
    ...
    el.innerHTML = snapshots.map(s=>`<option value="${s.id}">#${s.id} ${s.fetch_date}</option>`).join('');
